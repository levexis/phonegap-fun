		...
		if (action.equals(ACTION_INITIALIZE)) {
			...
			if (tagNames != null) {
				for (int i = 0; i < tagNames.length(); i++) {
					...
					LOG.d("Html5Video", "Init for Id: " + tagNames.getString(i) + " , src: " + convertedVideos.getString(tagNames.getString(i)));
				}
				callbackContext.sendPluginResult(new PluginResult(PluginResult.Status.OK, convertedVideos));				
				status = true;
 			} else {
 			    LOG.d("Html5Video", "no tagnames");
 			}
			...
			if (videoId != null) {
				cordova.getActivity().runOnUiThread(new Runnable() {
				     public void run() {
				           LOG.d("Html5Video", "calling _play with id: " + videoId);
				...
				LOG.d("Html5Video", "Not found video for id: " + videoId);
			}
